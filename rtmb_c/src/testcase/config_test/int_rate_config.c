 ...
 * Desc    : Configures benchmark for integral rate on the machine.
 * Args    : args - Test args.
 * Returns : NULL
 */

void *int_rate_config(void *args) {
	...
	if (result == NULL) {
		...
		RTMB_printf(stderr, " in int_rate_config()\n");
		abort();
	}

	strcpy((char *) result->desc,
	        (char *) "Integral rate configuration test");
	...
	RTMB_verbose_printf(stdout, cmd_args, 1,
	        "=====================================================\n");
	RTMB_verbose_printf(stdout, cmd_args, 1,
	        "\nint_rate_config : Total number of iterations = %d\n\n",
	        new_iters);
	...
	while (!done) {
		for (loop = 0; loop < new_iters; loop++) {
			...
			if (do_simple_int_opern(num) == ERROR) {
				free_chain(result, 0);
				targs->ret = ERROR;
				return (void *) NULL;
			}
			...
			RTMB_verbose_printf(stdout, cmd_args, 2,
			        "int_rate_config: Difference between end and"
				        " start times  = %.3f us \n", MICROSEC(
			                time_diff));

			/* Add the time_diff into the table */
		...
		if (IS_EXEC_TIME_GRT_THAN_CLK(result, clock_accuracy)) {
			...
			if (check_pass_criteria(result, cmd_args, bmtest, 0)
			        == SUCCESS) {
				/*  int rate is determined */
				done = 1;
				test_pass = ACCEPTABLE_DEVIATIONS;
				break;
			} else {
				if (++count == bmtest->_threshold) {
					RTMB_verbose_printf(stderr, cmd_args,
					        1, "int_rate_config: exceeded "
						        "maximum attempts\n");
	...
	add_result_to_result_table2(res_table, result, IRATE_CONFIG, bmtest);
