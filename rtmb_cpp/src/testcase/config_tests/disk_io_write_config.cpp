...
void *_disk_io_write_test_impl(void *);

/*
 * Name		: disk_io_write_test()
 * Desc		: Wrapper function for disk IO write configuration test
...
extern "C" bool disk_io_write_test(CmdLineParams &clParams,
   ...
   TestResults *tResult = new TestResults(DISKIOWRITE_CONFIG, new_iters);
   ...
   if (clParams.isVerbose() && (clParams.getVerboseLevel() >= 1)) {
      cout << endl << "Test Report for disk_io_write_test test:" << endl;
      cout << "====================================" << endl;
      cout << endl << "disk_io_write_test: Total number of "
         "iterations: " << new_iters << endl;
   }
   ...
   if (pthread_create(&tid, NULL, _disk_io_write_test_impl, (void *) &thr_args)
         == -1) {
      perror("disk_io_write_test");
      cerr << "ERROR: Thread creation failed in "
         "disk_io_write_test()" << endl;
      rc = false;
   } else {
...
/*
 * Name		: _disk_io_write_test_impl()
 * Desc		: Implements the disk IO write configuration test
 *		  as a separate RT thread.
 * Args		: args - Contains the test parameters.
 * Returns	: (void *) NULL.
 */

void *_disk_io_write_test_impl(void *args) {
   RTThreadArgs *threadArgs = (RTThreadArgs *) args;
   TestResults *tResult = threadArgs->tResult;

   int i = 0, count = 0;
   bool done(false);
   int num = DiskIOWriteUtils::getBlockSize();
   ...
   while (!done) {
      DiskIOWriteUtils *writeUtils = new DiskIOWriteUtils(num);

      for (i = 0; i < threadArgs->iterations; i++) {
         writeUtils -> openFileForWrite();

         TimeOperations::getCurTime(&startTime);

         for (int j = 0; j < subIters; j++) {
            if (writeUtils->doWrite() != num) {
         ...
         if (threadArgs->clParams.getVerboseLevel() >= 2) {
            cout << "disk_io_write_test: " << "Difference "
      ...
      delete tResult;
      tResult = new TestResults(DISKIOWRITE_CONFIG, (threadArgs->iterations));
