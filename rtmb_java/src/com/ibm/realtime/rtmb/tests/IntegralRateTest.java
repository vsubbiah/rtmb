...
public class IntegralRateTest extends SpecTest {
	...
	public TestResults[] benchMarkTest(TestInfo tInfo, RealTimeValues rtValues, TestParams tParams) {
		...
		int num = TestFinalDefinitions.INTRATE_CONFIG_NUM;
		...
		TestResults[] testResult = establishSingleResult(TestId.IRATE_CONFIG, tInfo.getTestIterations());
		...
		while ( true ) {
			...
			for (int loop = 0; loop < new_iters; loop++) {
				...
				MathOperations.do_simple_int_opern(num);
